...
#The goal is to analyze the reason/s for employees leaving the organization and to predict the employee who would 
#leave next based on the information available in the data. Hence, LEFT will be our respondent variable and the 
#other remaining variables can be seen as potential regressors.
...
mean(train$average_montly_hours)
hist(train$average_montly_hours)
cor(train$average_montly_hours,train$left) 
#Correlation gives an interesting insight that as this is a strong positive correlation, it means that people who have left the organisation have given more no. of average_montly_hours.

time_spend_company _left <- train[train$left == 1, c("time_spend_company", "left")]
time_spend_company_1 <- ggplot(time_spend_company_left, aes(time_spend_company, fill = factor(number_project))) + geom_bar()
time_spend_company_1

time_spend_company_exist <- train[train$left == 0, c("time_spend_company", "left")]
time_spend_company_0 <- ggplot(time_spend_company_exist, aes(time_spend_company, fill = factor(time_spend_company))) + geom_bar()
...
#Another interesting insight we draw here is that maximum employees have spent around 3 to 4 years in the company.
...
#The above two frequency tables show that the the maximum staff is employed in the sales department followed by technical and then the support team. The least no. of people are present in the management vertical. Also, the maximum employee strength is on Low salary, followed by Medium salary and very few employees have High salary. (almost 1/5th of the Medium salary). 
